...
public interface RoomRepository extends JpaRepository<Room, Long> {
    List<Room> findAllByMaxCapacityGreaterThanEqualAndCoworkingId(Integer capacity, Long coworkingId);
    Optional<Room> findByCoworkingIdAndId(Long coworkingId, Long roomId);

    @Query("SELECT r FROM Room r " +
            "WHERE r.coworking.id = :coworkingId " +
            "AND r.maxCapacity >= :minCapacity " +
            "AND NOT EXISTS (" +
    ...
    @Query("SELECT r FROM Room r " +
            ...
            "  AND (b.startTime < :endDateTime AND b.endTime > :startDateTime)" +
            ")")
    Optional<Room> getAvailableRoom(
